...
function RegisterServer()
{
    ...
    if( $_GET['profile'] !== NM_PROFILE || empty($_GET['desc']) || $_GET['port'] < 1 || $_GET['port'] > 65535 ) return "registering_failed";
    ...
    $ret = $mysqli->query( "INSERT INTO `netmatch` " .
                          "( `ip`, `port`, `version`, `devbuild`, `desc`) VALUES" .
                          "( '$ip', '$port', '$version', '$devbuild', '$desc' )"
                        );
    if( !$ret )
    {
        // query ei onnistunut. Kaadetaan homma t‰h‰n.
